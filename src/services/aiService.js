...
export const aiService = {
    ...
    analyzeFile: async (fileContent, fileName, modelName = null) => {
        try {
            ...
            File Content:
            ${fileContent.substring(0, 90000)} // Limit content to avoid token limits if necessary, though 1.5 pro has large context
            `;
    ...
    analyzeProject: async (files, modelName = null) => {
        try {
            ...
            let prompt = "Analyze the following project files and provide a high-level summary of the project, its architecture, and functionality.\n\n";

            for (const file of files) {
                // Skip non-text files or very large files for now
                if (file.type.startsWith('image') || file.size > 1000000) continue;
    ...
    queryWithContext: async (userQuery, files, projectName, modelName = null) => {
        try {
            const selectedModel = modelName ? getModel(modelName) : model;
            // System prompt for better formatting and responses
            let prompt = `You are an expert AI coding assistant analyzing the project "${projectName}".

IMPORTANT FORMATTING RULES:
- Use proper markdown formatting in your responses
- Wrap code snippets in triple backticks with language identifier (e.g., \`\`\`javascript, \`\`\`python, \`\`\`html)
...
- give the response in staright forward in one line , but if user ask details like that keyward then explain it in detail
